...
const Chatbot = () => {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end print:hidden">
            {/* Chat Window */}
            {isOpen && (
                <Card className="mb-4 w-[350px] h-[500px] flex flex-col shadow-2xl border border-primary/10 animate-fade-in-up origin-bottom-right overflow-hidden bg-white/95 backdrop-blur-md rounded-none">
                    {/* Header */}
                    <div className="bg-primary p-4 flex justify-between items-center text-primary-foreground">
                        ...
                        <Button
                            variant="ghost"
                            size="icon"
                            className="text-white hover:bg-white/10 h-8 w-8 rounded-none transition-colors"
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/20">
                                ...
                                >
                                    ...
                                    {msg.type === 'whatsapp-cta' && (
                                        <div className="mt-4 pt-4 border-t border-border/50">
                                            <Button
                                                className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white shadow-none rounded-none transition-all hover:translate-x-1 font-medium tracking-wide"
                        ...
                        {isTyping && (
                            <div className="flex justify-start">
                                <div className="bg-white shadow-sm border border-border p-4">
                    ...
                    <div className="p-4 bg-white border-t border-border">
                        ...
                        >
                            <Input
                                ...
                                className="flex-1 bg-muted/30 border-border focus-visible:ring-primary rounded-none"
                            />
                            <Button
                                type="submit"
                                size="icon"
                                disabled={!inputValue.trim() || messages.some(m => m.type === 'whatsapp-cta')}
                                className="bg-primary text-white hover:bg-black rounded-none w-10 h-10 transition-transform active:scale-95"
                            >
            ...
            {!isOpen && showTooltip && (
                <div
                    className="absolute bottom-24 right-0 bg-primary text-primary-foreground px-5 py-4 shadow-xl border border-white/10 animate-fade-in-up cursor-pointer flex items-center whitespace-nowrap z-40 hover:-translate-x-1 transition-transform max-w-[300px]"
                    onClick={() => {
                        setIsOpen(true);
                        setShowTooltip(false);
                    }}
                >
                    <div className="w-10 h-10 bg-white/10 flex items-center justify-center mr-4 flex-shrink-0 border border-white/20">
                        <HardHat className="w-5 h-5 text-white" />
                    </div>
                    <div>
                    ...
                    <button
                        ...
                        className="absolute -top-2 -right-2 bg-white text-primary w-6 h-6 flex items-center justify-center shadow-md hover:bg-accent hover:text-white transition-colors"
                    >
                        <X className="w-3 h-3" />
                    </button>
                </div>
            ...
            <div className="relative group">
                ...
                {!isOpen && (
                    <span className="absolute inline-flex h-full w-full bg-primary opacity-10 animate-ping duration-[3000ms]"></span>
                )}

                <Button
                    onClick={() => setIsOpen(!isOpen)}
                    size="lg"
                    className={`w-16 h-16 shadow-2xl relative z-10 transition-all duration-500 rounded-none ${isOpen
